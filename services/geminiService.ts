...
- Больові відчуття (втома): ${wellnessCheck.fatigue}/10${wellnessCheck.notes ? `\n- Нотатки: "${wellnessCheck.notes}"` : ''}${userProfile.healthConstraints && userProfile.healthConstraints.length > 0 ? `\n- Постійні обмеження здоров'я: ${userProfile.healthConstraints.join(', ')}` : ''}${smartContext}
...
4.  **Обмеження здоров'я:** Постійні обмеження здоров'я (healthConstraints) мають абсолютний пріоритет. НІКОЛИ не пропонуй вправи, які можуть зашкодити цим обмеженням.
5.  **Логіка:**
    *   КОЛИ ЕНЕРГІЯ НИЗЬКА: зменш обсяг (підходи/повторення) на 20-40%, збільш відпочинок.
    *   КОЛИ СОН ПОГАНИЙ: зменш інтенсивність, трохи збільш відпочинок.
    *   КОЛИ СТРЕС ВИСОКИЙ: фокус на техніці, не на вазі. Можна замінити складні вправи на простіші.
    *   КОЛИ ВТОМА ВИСОКА: значно зменш навантаження, можливо, запропонуй легке кардіо або розтяжку замість силових.
    *   КОЛИ ВСІ ПОКАЗНИКИ ВИСОКІ: дотримуйся плану, можливо, трохи збільш вагу або повторення.
