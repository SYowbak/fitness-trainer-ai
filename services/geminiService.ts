...
import { UserProfile, DailyWorkoutPlan, Exercise, MuscleGroup } from '../types';
import { 
  getUkrainianGoal, 
...
export const generateSingleExercise = async (
  ...
  const prompt = `Ти — висококваліфікований персональний фітнес-тренер. Створи ОДНУ вправу для тренування ${getUkrainianMuscleGroup(targetMuscleGroup)}.

**Вхідні дані користувача:**
*   Стать: ${getUkrainianGender(profile.gender)}
*   Вік: ${profile.age} років
*   Тип статури: ${getUkrainianBodyType(profile.bodyType)}
*   Головна фітнес-ціль: ${getUkrainianGoal(profile.goal)}
*   Рівень підготовки: ${getUkrainianExperienceLevel(profile.experienceLevel)}
*   Зріст: ${profile.height} см
*   Вага: ${profile.weight} кг

**Вимоги до вправи:**
1. **Підбір:** Вправа має відповідати статі, типу статури, цілі та рівню підготовки користувача.
2. **Назва:** Вкажи точну українську назву вправи.
3. **Опис Техніки:** Надай ДУЖЕ ДЕТАЛЬНИЙ, покроковий опис правильної техніки виконання вправи. Включи:
   * Початкове положення.
   * Ключові моменти руху.
   * Правильне дихання.
   * Типові помилки та як їх уникнути.
   * Поради для максимальної ефективності.
4. **Кількість підходів:** Вкажи кількість робочих підходів (наприклад, "3-4" або число 4).
5. **Кількість повторень:** Вкажи діапазон повторень, оптимальний для цілі (наприклад, "8-12" для гіпертрофії, "12-15" для витривалості).
6. **Відпочинок:** Вкажи рекомендований час відпочинку між підходами в секундах (наприклад, "60-90 секунд").
7. **imageSuggestion:** Надай коротку текстову пропозицію для статичного зображення або GIF, яке б чітко ілюструвало КЛЮЧОВИЙ МОМЕНТ або правильну форму вправи. Якщо ідеї немає, залиш null.
8. **videoSearchQuery:** Надай точний пошуковий запит для YouTube, який допоможе користувачеві знайти якісне відео з демонстрацією техніки вправи українською або російською мовою. Якщо якісного запиту сформулювати не вдається, залиш null.

**Формат відповіді:**
Надай відповідь ВИКЛЮЧНО у форматі JSON об'єкта. Не додавай жодних пояснень, коментарів або тексту поза JSON структурою. JSON має бути ідеально валідним.
...
**Приклад відповіді:**
{
  ...
  try {
    ...
    try {
      const parsedExercise: any = JSON.parse(jsonStr);
      
      // Перевіряємо обов'язкові поля
      if (!parsedExercise.name || !parsedExercise.description || !parsedExercise.sets || !parsedExercise.reps || !parsedExercise.rest) {
        throw new Error("Відсутні обов'язкові поля у відповіді AI");
      }
      ...
      return {
        ...
        rest: parsedExercise.rest,
        imageSuggestion: parsedExercise.imageSuggestion || null,
        videoSearchQuery: parsedExercise.videoSearchQuery || null,
        targetWeight: null,
        targetReps: null,
        isCompletedDuringSession: false,
        sessionLoggedSets: [],
      ...
      console.error("Error parsing JSON from AI response:", e);
      console.error("Received string (after processing):", jsonStr);
      throw new Error("Не вдалося розібрати вправу від AI. Можливо, формат відповіді змінився, або сталася помилка на стороні AI.");
    }
  } catch (error: any) {
    console.error("Error during exercise generation via Gemini API:", error);
    if (error.message && (error.message.includes("API_KEY_INVALID") || (error.response && error.response.status === 400))) {
      throw new Error("Наданий API ключ недійсний або не має дозволів. Будь ласка, перевірте ваш API ключ.");
    }
    throw new Error(`Помилка генерації вправи: ${error.message || 'Невідома помилка сервісу AI'}`);
  }
};